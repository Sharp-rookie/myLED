...
def ID_subworker(args, tau, random_seed=729, is_print=False):
    ...
    test_and_save_embeddings_of_time_lagged(args.system, args.embedding_dim, args.channel_num, args.obs_dim, tau, args.id_epoch, args.id_log_dir+f"tau_{tau}/seed{random_seed}", is_print, random_seed, args.data_dir, args.id_log_dir, args.device, args.data_dim, args.batch_size, args.enc_net, args.e1_layer_n, sliding_window, args.tau_unit, args.total_t)
    ...
    for tau in T:
        ...
        if args.sample_type == 'sliding_window':
            ...
            if 'PNAS17' not in args.system:
                ...
                xdim = int(args.system.split('xdim')[-1])
                generate_dataset_static(1000, tau=tau, dt=args.dt, max_tau=args.tau_N, is_print=True, parallel=args.parallel, xdim=xdim, data_dir=args.data_dir)
        else:
...
if __name__ == '__main__':
    ...
    if args.system == '2S2F':
        ...
        from Data.generator_pnas17 import generate_dataset_slidingwindow, generate_dataset_static, generate_original_data
        from util.plot_pnas17 import plot_epoch_test_log, plot_id_per_tau, plot_autocorr
