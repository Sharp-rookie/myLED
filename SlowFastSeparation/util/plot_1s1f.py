...
def plot_autocorr(T, dt=0.01):
    ...
    data = np.load('Data/1S1F/origin/1/data.npz')
    ...
    np.savez('Data/1S1F/autocorr.npz', corrX=corrX, corrY=corrY)


def plot_evolve(length):
    
    our = open(f'results/1S1F/evolve_test_{length}.txt', 'r')
    lstm = open(f'results/1S1F/lstm_evolve_test_{length}.txt', 'r')
    tcn = open(f'results/1S1F/tcn_evolve_test_{length/5}.txt', 'r')
