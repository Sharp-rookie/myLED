...
def plot_epoch_test_log(tau, max_epoch):
    ...
    fp = open(f'logs/1S1F/TimeSelection/tau_{tau}/test_log.txt', 'r')
    items = []
    ...
    plt.legend()
    plt.savefig(f'logs/1S1F/TimeSelection/tau_{tau}/ID_per_epoch.pdf', dpi=300)
    plt.close()
...
def plot_id_per_tau(tau_list, id_epoch):
    ...
    for i, tau in enumerate(tau_list):
        fp = open(f'logs/1S1F/TimeSelection/tau_{round(tau,2)}/test_log.txt', 'r')
        for line in fp.readlines():
            seed = int(line[:-1].split(',')[1])
            epoch = int(line[:-1].split(',')[4])
            MLE_id = float(line[:-1].split(',')[5])
    ...
    plt.savefig('logs/1S1F/TimeSelection/id_per_tau.pdf', dpi=300)
...
def plot_autocorr(T, dt=1.):
    ...
    if os.path.exists('Data/1S1F/autocorr.pdf'): return

    simdata = np.load('Data/1S1F/origin/origin.npz')
    ...
    for trace_id in range(trace_num):
        ...
        fig.colorbar(im)
        plt.savefig('Data/1S1F/corr.pdf', bbox_inches='tight')
    ...
    plt.savefig('Data/1S1F/autocorr.pdf', dpi=300)

    np.savez('Data/1S1F/autocorr.npz', corrX=corrX, corrY=corrY, corrU=corrU, corrV=corrV)
    
    
def plot_evolve(length):
    
    our = open(f'results/1S1F/pretrain100_evolve_test_{length}.txt', 'r')
    lstm = open(f'results/1S1F/lstm_evolve_test_{length}.txt', 'r')
    tcn = open(f'results/1S1F/tcn_evolve_test_{length}.txt', 'r')
    ode = open(f'results/1S1F/neuralODE_evolve_test_{length}.txt', 'r')
    ...
    for i, item in enumerate(['mse', 'rmse', 'mae', 'mape']):
        ...
        ax.set_xlabel('t / s')
        ax.legend()
    plt.savefig(f'results/1S1F/evolve_test_{length}.pdf', dpi=300)
    ...
    for i, item in enumerate(['RMSE', 'MAPE']):
        ...
        plt.xticks(fontsize=16)
        plt.yticks(fontsize=16)
        plt.savefig(f'results/1S1F/evolve_comp_{item}.pdf', dpi=300)
    ...
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    plt.savefig(f'results/1S1F/slow_evolve_mae.pdf', dpi=300)
    ...
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    plt.savefig(f'results/1S1F/our_slow_evolve_mae.pdf', dpi=300)
