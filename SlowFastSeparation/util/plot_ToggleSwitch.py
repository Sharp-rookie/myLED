...
def plot_epoch_test_log(tau, max_epoch, log_dir):
    ...
    class MSE():
        def __init__(self, tau):
            self.tau = tau
            self.mse_Px = [[] for _ in range(max_epoch)]
            self.mse_Py = [[] for _ in range(max_epoch)]
    ...
    for line in fp.readlines():
        ...
        mse_Px = float(line[:-1].split(',')[2])
        mse_Py = float(line[:-1].split(',')[3])
        ...
        for M in items:
            if M.tau == tau:
                M.mse_Px[epoch].append(mse_Px)
                M.mse_Py[epoch].append(mse_Py)
    ...
    for M in items:
        ...
        for epoch in epoches:
            mse_Px_list.append(np.mean(M.mse_Px[epoch]))
            mse_Py_list.append(np.mean(M.mse_Py[epoch]))
    ...
    plt.ylabel('MSE')
    name = ['P_x', 'P_y']
    plt.plot(epoches, mse_Px_list, label=name[0])
    plt.plot(epoches, mse_Py_list, label=name[1])
...
def plot_autocorr(T, dt=0.01):
    ...
    if os.path.exists('Data/ToggleSwitch/autocorr.pdf'): return

    data = np.load('Data/ToggleSwitch/origin/1/data.npz')
    ...
    fig.colorbar(im)
    plt.savefig('Data/ToggleSwitch/corr.pdf', bbox_inches='tight')
    ...
    plt.savefig('Data/ToggleSwitch/autocorr.pdf', dpi=300)

    np.savez('Data/ToggleSwitch/autocorr.npz', corrX=corrX, corrY=corrY, corrZ=corrZ)


def plot_evolve(length):
    
    our = open(f'results/ToggleSwitch/evolve_test_{length}.txt', 'r')
    lstm = open(f'results/ToggleSwitch/lstm_evolve_test_{length}.txt', 'r')
    tcn = open(f'results/ToggleSwitch/tcn_evolve_test_{length/5}.txt', 'r')
    ode = open(f'results/ToggleSwitch/neuralODE_evolve_test_{length}.txt', 'r')
    ...
    for i, item in enumerate(['mse', 'rmse', 'mae', 'mape']):
        ...
        ax.set_xlabel('t / s')
        ax.legend()
    plt.savefig(f'results/ToggleSwitch/evolve_test_{length}.pdf', dpi=300)
