...
def plot_autocorr(T, dt=1.):
    ...
    if os.path.exists('Data/HalfMoon/autocorr.pdf'): return

    simdata = np.load('Data/HalfMoon/origin/origin.npz')
    ...
    for trace_id in range(trace_num):
        ...
        fig.colorbar(im)
        plt.savefig('Data/HalfMoon/corr.pdf', bbox_inches='tight')
    ...
    plt.savefig('Data/HalfMoon/autocorr.pdf', dpi=300)

    np.savez('Data/HalfMoon/autocorr.npz', corrX=corrX, corrY=corrY, corrU=corrU, corrV=corrV)
    
    
def plot_evolve(length):
    
    our = open(f'results/HalfMoon/pretrain100_evolve_test_{length}.txt', 'r')
    lstm = open(f'results/HalfMoon/lstm_evolve_test_{length}.txt', 'r')
    tcn = open(f'results/HalfMoon/tcn_evolve_test_{length}.txt', 'r')
    ode = open(f'results/HalfMoon/neuralODE_evolve_test_{length}.txt', 'r')
    ...
    for i, item in enumerate(['mse', 'rmse', 'mae', 'mape']):
        ...
        ax.set_xlabel('t / s')
        ax.legend()
    plt.savefig(f'results/HalfMoon/evolve_test_{length}.pdf', dpi=300)
    ...
    for i, item in enumerate(['RMSE', 'MAPE']):
        ...
        plt.xticks(fontsize=16)
        plt.yticks(fontsize=16)
        plt.savefig(f'results/HalfMoon/evolve_comp_{item}.pdf', dpi=300)
    ...
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    plt.savefig(f'results/HalfMoon/slow_evolve_mae.pdf', dpi=300)
    ...
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    plt.savefig(f'results/HalfMoon/our_slow_evolve_mae.pdf', dpi=300)
