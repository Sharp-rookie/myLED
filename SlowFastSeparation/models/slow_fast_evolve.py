...
class DynamicsEvolver(nn.Module):
    ...
    def obs2slow(self, obs):
        ...
        # h0 = torch.zeros(self.layer_num * 1, len(obs), self.hidden_dim, dtype=torch.float32, device=self.device)
        # c0 = torch.zeros(self.layer_num * 1, len(obs), self.hidden_dim, dtype=torch.float32, device=self.device)
        # x = nn.Flatten(start_dim=2)(obs)
        # _, (h, c)  = self.encoder_1(x, (h0, c0))
        # embed = h[-1]
