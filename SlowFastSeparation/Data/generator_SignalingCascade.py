...
from .gillespie import generate_signalling_cascade_origin
...
def time_discretization(seed, total_t, dt=None, is_print=False, data=None, save=True):
    ...
    if not data:
        data = np.load(f'Data/SignalingCascade/origin/{seed}/origin.npz')
    ...
    while current_t < total_t:
        ...
        t.append(current_t)
        X1.append(data_X1[index])
        X2.append(data_X2[index])
        X3.append(data_X3[index])
        X4.append(data_X4[index])
    ...
    if save:
        ...
        plt.plot(t, X3, label=r'$X3$')
        plt.plot(t, X4, label=r'$X4$')
        plt.xlabel(r'$t / s$', fontsize=18)
        plt.legend()
        plt.xticks(fontsize=15)
        plt.yticks(fontsize=15)
        plt.savefig(f'Data/SignalingCascade/origin/{seed}/data.pdf', dpi=300)

        np.savez(f'Data/SignalingCascade/origin/{seed}/data.npz', dt=dt, t=t, X1=X1, X2=X2, X3=X3, X4=X4)
    else:
...
def generate_original_data(trace_num, total_t, dt, save=True, plot=False, parallel=False):
    ...
    os.makedirs('Data/SignalingCascade/origin', exist_ok=True)

    # generate original data by gillespie algorithm
    subprocess = []
    for seed in range(1, trace_num+1):
        if not os.path.exists(f'Data/SignalingCascade/origin/{seed}/origin.npz'):
            IC = [0, 0, 0, 0]
            if parallel:
                is_print = len(subprocess)==0
                subprocess.append(Process(target=generate_signalling_cascade_origin, args=(total_t, seed, IC, True, is_print), daemon=True))
                subprocess[-1].start()
            else:
                generate_signalling_cascade_origin(total_t, seed, IC, is_print=True)
    ...
    subprocess = []
    for seed in range(1, trace_num+1):
        if not os.path.exists(f'Data/SignalingCascade/origin/{seed}/data.npz'):
            is_print = len(subprocess)==0
            if parallel:
    ...
    print(f'save origin data form seed 1 to {trace_num} at Data/SignalingCascade/origin/')


def generate_original_data2(trace_num, total_t, dt, parallel=False):

    def unit(queue, total_t, seed, dt, IC, is_print):
        simdata = generate_signalling_cascade_origin(total_t=total_t, seed=seed, IC=IC, save=False, is_print=is_print)
...
def generate_dataset_static(trace_num, tau=0., dt=0.01, max_tau=5., is_print=False, parallel=False):
    ...
    if os.path.exists(f"Data/SignalingCascade/data/tau_{tau}/train_static.npz") and os.path.exists(f"Data/SignalingCascade/data/tau_{tau}/val_static.npz") and os.path.exists(f"Data/SignalingCascade/data/tau_{tau}/test_static.npz"):
        return
    
    # generate simulation data
    if not os.path.exists(f"Data/SignalingCascade/data/static_{max_tau:.2f}.npz"):
        ...
        np.savez(f"Data/SignalingCascade/data/static_{max_tau:.2f}.npz", data=data)
        if is_print: print(f'tau[{tau}]', 'data shape', data.shape, '# (trace_num, time_length, channel, feature_num)')
    else:
        data = np.load(f"Data/SignalingCascade/data/static_{max_tau:.2f}.npz")['data']

    if is_print: print(f"\r[{tau}/{max_tau}]", end='')

    # save statistic information
    data_dir = f"Data/SignalingCascade/data/tau_{tau}"
    ...
    for item in ['train','val','test']:
        ...
        plt.plot(sequences[:,0,0,0], label='X1')
        plt.plot(sequences[:,0,0,1], label='X2')
        plt.plot(sequences[:,0,0,2], label='X3')
        plt.plot(sequences[:,0,0,3], label='X4')
        plt.legend()
        plt.savefig(data_dir+f'/{item}_static_input.pdf', dpi=300)

        plt.figure(figsize=(16,10))
        plt.title(f'{item.capitalize()} Data')
        plt.plot(sequences[:,squence_length-1,0,0], label='X1')
        plt.plot(sequences[:,squence_length-1,0,1], label='X2')
        plt.plot(sequences[:,squence_length-1,0,2], label='X3')
        plt.plot(sequences[:,squence_length-1,0,3], label='X4')
        plt.legend()
...
def generate_dataset_slidingwindow(trace_num, tau, sample_num=None, is_print=False, sequence_length=None):
    ...
    if (sequence_length is not None) and os.path.exists(f"Data/SignalingCascade/data/tau_{tau}/train_{sequence_length}.npz") and os.path.exists(f"Data/SignalingCascade/data/tau_{tau}/val_{sequence_length}.npz") and os.path.exists(f"Data/SignalingCascade/data/tau_{tau}/test_{sequence_length}.npz"):
        return
    elif (sequence_length is None) and os.path.exists(f"Data/SignalingCascade/data/tau_{tau}/train.npz") and os.path.exists(f"Data/SignalingCascade/data/tau_{tau}/val.npz") and os.path.exists(f"Data/SignalingCascade/data/tau_{tau}/test.npz"):
        return
    ...
    for trace_id in iter:
        tmp = np.load(f"Data/SignalingCascade/origin/{trace_id}/data.npz")
        dt = tmp['dt']
        X1 = np.array(tmp['X1'])[:, np.newaxis, np.newaxis] # (sample_num, channel, feature_num)
        X2 = np.array(tmp['X2'])[:, np.newaxis, np.newaxis]
        X3 = np.array(tmp['X3'])[:, np.newaxis, np.newaxis]
        X4 = np.array(tmp['X4'])[:, np.newaxis, np.newaxis]

        trace = np.concatenate((X1, X2, X3, X4), axis=-1)
    ...
    for item in ['train','val','test']:
        ...
        if seq_none:
            ...
            plt.figure(figsize=(16,10))
            plt.title(f'{item.capitalize()} Data' + f' | sample_num[{len(sequences) if sample_num is None else sample_num}]')
            plt.plot(sequences[:, sequence_length-1, 0, 0], label='X1')
            plt.plot(sequences[:, sequence_length-1, 0, 1], label='X2')
            plt.plot(sequences[:, sequence_length-1, 0, 2], label='X3')
            plt.plot(sequences[:, sequence_length-1, 0, 3], label='X4')
            plt.legend()
