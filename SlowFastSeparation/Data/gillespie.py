    ...
    def evolute(self, steps=None, total_t=None, IC=None, seed=1, is_print=False):
        ...
        if steps is not None:
            ...
            while self.t[-1] < total_t:
                ...
                d = np.random.choice(self.reactions, p=A)
                self.n.append(self.n[-1] + d.num_diff)
                if is_print: print(f'\rSeed[{seed}] time: {self.t[-1]:.3f}/{total_t}s | X={self.n[-1][0]}, Y={self.n[-1][1]}, Z={self.n[-1][2]}', end='')
...
def generate_origin(total_t=None, seed=729, IC=[100,40,2500], save=True, is_print=False):
