...
class FitzHugh_Nagumo():
    ...
    def __call__(self, y0, t):
        ...
        du = u - u**3 - v
        dv = self.epsilon * (u - self.a1*v - self.a0)
...
def generate_original_data(trace_num, total_t=20.1, dt=0.01, save=True, plot=False, parallel=False):
    ...
    def solve_1_trace(trace_id=0, total_t=5, dt=0.01):
        ...
        # y0 = [np.random.uniform(-1,1), np.random.uniform(-1,1)]
        y0 = [1., 0.1]

        t  =np.arange(0, total_t, dt)
        a0, a1, epsilon = -0.03, 2., 0.006

        sol = odeint(FitzHugh_Nagumo(a0,a1,epsilon), y0, t)
    ...
    print(f'save origin data from seed 1 to {trace_num} at Data/origin/')
