...
def baseline_train(
    ...
    # prepare
    data_filepath = data_dir + 'tau_' + str(tau_unit)
    ...
    # dataset
    train_dataset = Dataset(data_filepath, 'train', length=int(tau_s/tau_unit))
        ...
        for input, _, internl_units in train_loader:
            ...
            for i in range(1, len(internl_units)):
                ...
                if model.__class__.__name__ == 'LSTM':
                    ...
                    t = torch.tensor([tau_unit], device=device)
        ...
        with torch.no_grad():
            ...
            for input, target in val_loader:
                ...
                if model.__class__.__name__ == 'LSTM':
                    ...
                    t = torch.tensor([tau_s-tau_unit], device=device)
                    output = model(input, t)
