...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, input_1d_width, latent_dim=4, nhidden=20):
...
class Latent_ODE(nn.Module):
    ...
    def __init__(self, in_channels, input_1d_width, latent_dim=4, nhidden=20, rnn_nhidden=25):
...
def train(tau, delta_t, is_print=False, random_seed=729):
    ...
    for epoch in range(1, max_epoch+1):
        ...
        for _, (input, target) in enumerate(train_loader):
            input = model.scale(input[0].to(device)) # (batchsize,1,1,3)
            target = model.scale(target[0].to(device))
        ...
        with torch.no_grad():
            ...
            # cal loss
            if is_print: print(f'\rTau[{tau}] | epoch[{epoch}/{max_epoch}] | val-loss={loss:.5f}', end='')
...
def test_evolve(tau, ckpt_epoch, delta_t, n, is_print=False, random_seed=729):
    ...
    with torch.no_grad():
        ...
        for input, target in test_loader:
            input = model.scale(input[0].to(device))
            target = model.scale(target[0].to(device))
