...
#                                        exec sh";
screen -S eval-"$dataset" -dm bash -c "CUDA_VISIBLE_DEVICES="$gpu" python ../eval.py ../configs/"$dataset"/model/config1.yaml ../logs/logs_"$dataset"_encoder-decoder_1/lightning_logs/checkpoints NA eval-train NA; \
                                       exec sh";