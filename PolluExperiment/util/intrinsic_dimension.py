...
class ID_Estimator:
    def __init__(self, method='MLE'):
        self.all_methods = ['MiND_ML', 'MLE', 'MADA', 'PCA', 'MOM', 'ESS', 'TLE', 'CorrInt', 'DANCo']
    ...
    def fit(self, X, k_list=20):
        ...
        for k in k_list:
            if self.method == 'MiND_ML':
                ...
                dims.append(np.mean(skdim.id.ESS().fit(X, n_neighbors=k).dimension_pw_))
                ...
                dims.append(np.mean(skdim.id.CorrInt().fit_pw(X, n_neighbors=k).dimension_pw_))
            else:
...
def eval_id_embedding(vars_filepath, method='MLE', is_print=False, max_point=1000):
    ...
    k_list = (embedding.shape[0] * np.linspace(0.05, 0.10, 5)).astype('int')
