...
def plotScheduleLoss(model, ifp_train_vec, ifp_val_vec):
    fig_path = getFigureDir(model) + "/schedule_loss." + FIGTYPE
    fig, ax = plt.subplots(figsize=(20, 10))
...
def createIterativePredictionPlots(model, target, prediction, dt, ic_idx, set_name, \
    ...
    if model.params["Dx"] > 0 and model.params["Dy"] > 0:
        ...
        if ((target_augment is not None) and (prediction_augment is not None)):
            ...
            # fig_path = getFigureDir(model) + "/{:}_augmented_{:}_{:}.{:}".format(testing_mode, set_name,
            #                                         ic_idx, FIGTYPE)
            ...
            # fig_path = getFigureDir(model) + "/{:}_{:}_{:}.{:}".format(
            #     testing_mode, set_name, ic_idx, FIGTYPE)
...
def makeVideoPaths(model, video_folder, field=""):
    n_frames_max = 100
    # n_frames_max = 1000
    video_base_dir = getFigureDir(model)
...
def makeVideo(model, video_path, frame_path_bash, n_frames_max):
    ...
    with open(
            getFigureDir(model) + "/video_commands_abs.sh",
            "a+") as file:
        file.write(command_str)
        file.write("\n")

    # Remove duplicates
    filename = getFigureDir(model) + "/video_commands_abs.sh"
    ...
    with open(
            getFigureDir(model) + "/video_commands_rel.sh",
            "a+") as file:
    ...
    # Remove duplicates
    filename = getFigureDir(model) + "/video_commands_rel.sh"
    removeDuplicates(filename)
...
#     # fig_path = getFigureDir(model) + "/{:}_state_dist_S{:}_{:}.{:}".format(testing_mode, state_num, set_name, FIGTYPE)
#     # bin_width = bin_centers[1] - bin_centers[0]
#     # vmax_density = np.max([np.max(density_target), np.max(density_pred)])
#     # vmin_density = 0.0
#     # fig, ax = plt.subplots(figsize=(6, 6))
