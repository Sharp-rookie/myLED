...
def writeLogfiles(model, testing_mode=None):
    ...
    if model.write_to_log:
        ...
        for set_name in write_logs_on:
            data_path = utils.getResultsDir(model) + "/results_{:}_{:}".format(testing_mode, set_name)
            results = utils.loadData(data_path, model.save_format)
            # Writing to a log-file
            logfile = utils.getLogFileDir(model) + "/results_{:}_{:}.txt".format(testing_mode, set_name)
            utils.writeToLogFile(model, logfile, results, results["fields_2_save_2_logfile"])
            ...
            if model.params["plot_testing_ics_examples"]:
                ...
                for ic in range(ic_plot):
                    ...
                    if model.params["plot_latent_dynamics"]:
                        utils.plotLatentDynamics(model, set_name, results["latent_states_all"][ic], ic, testing_mode)
