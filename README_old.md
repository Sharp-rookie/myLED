...
- [Discovering State Variables Hidden in Experimental Data](#discovering-state-variables-hidden-in-experimental-data)
    - [Project Website | Video | Paper](#project-website--video--paper)
  - [Overview](#overview)
  - [Citation](#citation)
  - [Content](#content)
...
5. Train the neural latent dynamics model (latentpred) with the trained models from previous steps.
    ```
    ./latentpred_train.sh {dataset_name} {gpu no.}
    ```
...
3. Calculate the final intrinsic dimension estimated values (mean and standard deviation).
    ```
    PYTHONIOENCODING=utf-8 python ../utils/dimension.py {dataset_name}
    ```
