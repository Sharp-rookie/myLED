...
def getAutoencoderArchitecture(model):
    ...
    if system_name == "FHN":
    ...
    elif system_name == "FHNStructured":
        ...
        raise ValueError("System name {:} not recognized. Do not know how to build CNN-MLP Autoencoder.".format(system_name))
...
def getEncoderModel(model, mode):
...
def getDecoderModel(model, encoder, mode):
    
    params_dict = getAutoencoderArchitecture(model)

    if 'only_inhibitor' in mode or 'only_activator'in mode:
        assert params_dict["input_chanels"]==1
    ...
    decoder = ConvMLPDecoder(
        ...
        input_chanels=2 if ('only_inhibitor' in mode) or ('only_activator'in mode) else params_dict["input_chanels"],
        input_image_width=params_dict["input_image_width"],
        dropout_keep_prob=params_dict["dropout_keep_prob"],
    )
...
def getSamePadding(stride, image_size, filter_size):
    ...
    if half_pad % 2 == 1:
        raise ValueError("(S-1) * W  - S + F has to be divisible by two ({:}-1)*{:} - {:} + {:} = {:}".format(S, W, S, F, half_pad))
    else:
        pad = int(half_pad / 2)
    
    if (pad > image_size / 2):
        raise ValueError("Very large padding P={:}, compared to input width {:}. Reduce the strides.".format(pad, image_size))
...
class ConvMLPEncoder(nn.Module):
    def __init__(
        ...
        if not self.input_image_width:
            raise ValueError("In case of AE reducing the dimensionality, the width of the input image has to be provided.")

        #############################################
        # Calculate Padding Size
        #############################################
        # Padding for inputwidth
        if not isPowerOfTwo(self.input_image_width):
            # Add padding in the first layer to make power of two
            self.input_image_width_padded = findNextPowerOfTwo(self.input_image_width)
        ...
        # Padding for middle layers
        if self.zero_padding:
            self.conv_layers_zero_padding = []
            for i in range(len(self.conv_layers_kernel_sizes)): # conv_layers_kernel_sizes: [5, 5, 5, 5]
                pad_x = getSamePadding(
                    self.conv_layers_strides[i],
                    self.input_image_width_padded,
                    self.conv_layers_kernel_sizes[i],
                )
                self.conv_layers_zero_padding.append(tuple([pad_x, pad_x])) # (padding_left, padding_right)

        if self.pool_type not in ["max", "avg"]:
            raise ValueError("Invalid pooling operation type {:}.".format(self.pool_type))
        ...
        for i in range(len(self.conv_layers_kernel_sizes)):
            ...
            # Padding
            if self.zero_padding:
                self.layers.append(nn.ConstantPad1d(self.conv_layers_zero_padding[i], 0.0))
                self.layers_num += 1
            ...
            if self.batch_norm:
                if i < len(self.conv_layers_kernel_sizes) - 1:
                    self.layers.append(nn.BatchNorm1d(self.conv_layers_channels[i + 1], affine=False))
                    self.layers_num += 1

            # Pooling
            if self.pool_type == "max":
                self.layers.append(nn.MaxPool1d(self.pool_kernel_sizes[i], stride=self.pool_kernel_strides[i]))
                self.layers_num += 1
            elif self.pool_type == "avg":
                self.layers.append(nn.AvgPool1d(self.pool_kernel_sizes[i], stride=self.pool_kernel_strides[i]))
                self.layers_num += 1
            else:
        ...
        self.state_dim_before_mlp = sizes[-1]
        ...
        self.layers_num += 1
        self.layers.append(torch.nn.Linear(in_features, out_features, bias=True))
        self.layers_num += 1
        self.layers_idx.append(self.layers_num - 1)
    ...
    def getDimensions(self):
        ...
        input = torch.randn(1, self.input_chanels, self.input_image_width,)
        sizes_all = []
