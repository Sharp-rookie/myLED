    ...
    def forward(self, input, state, is_train):
        ...
        if isinstance(self.RNN_cell, nn.LSTMCell):
            ...
            if is_train and self.zoneout_prob < 1.0:
                h, c = state
                mask = torch.bernoulli(self.zoneout_prob * torch.ones_like(h_new)) / self.zoneout_prob
            ...
            if is_train and self.zoneout_prob < 1.0:
                mask = torch.bernoulli(self.zoneout_prob * torch.ones_like(state_new)) / self.zoneout_prob
