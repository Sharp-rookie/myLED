...
def addResultsSystemFHN(model, results, testing_mode):
    ...
    if model.data_info_dict["structured"]:
        ...
        predictions_all = utils.getDataFromHDF5PathArrayMultipleICs(predictions_all)
    ...
    mnad_in = np.mean(np.abs(targets_all_in - predictions_all_in) / (np.max(targets_all_in) - np.min(targets_all_in)), axis=2)

    mnad_in_avg = np.mean(mnad_in)
    print("(MNAD) Mean normalised absolute difference on the inhibitor density: {:}".format(mnad_in_avg))
